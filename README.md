...
- **Cloudflare Pages / Netlify / Vercel** — connect the repo; build command `npm run build`,
  output directory `dist`. Or skip the build entirely and upload `dist/` directly.
- **GitHub Pages** — serve from `dist/` on the default branch.
- **Anywhere else** — copy `dist/` onto the host, or open `dist/index.html` locally.
...
This is the production client of the full-stack VRGTrack app (React + tRPC + Drizzle/MySQL)
with only the network layer replaced:
